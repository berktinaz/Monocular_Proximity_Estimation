...
class InferenceHelper:
    def __init__(self, dataset='nyu', device='cuda:0'):
        ...
        if dataset == 'nyu':
            ...
            pretrained_path = "/data/btinaz/mono_depth/AdaBins/checkpoints/TrainScratch_29-Jul_22-03-nodebs12-tep25-lr0.0005-wd0.1-5102f7cf-fc7e-4dc3-b666-ebfdaea9c087_best.pt" #scratch 0.0005
            #pretrained_path = "/data/btinaz/mono_depth/AdaBins/checkpoints/TrainScratch_30-Jul_12-46-nodebs12-tep25-lr0.001-wd0.1-4e164701-08dd-4ec9-a4b8-dc5b43d07ce7_best.pt" #scratch 0.001
            #pretrained_path = "./pretrained/AdaBins_nyu.pt" #baseline
...
if __name__ == '__main__':
    ...
    img = Image.open("/data/btinaz/CARE_sample/sample.png")
    img = img.resize((640,480))
    start = time()
    inferHelper = InferenceHelper()
    centers, pred = inferHelper.predict_pil(img)
    print(f"took :{time() - start}s")
    # print(pred.squeeze(), max(pred.squeeze()))
    pred = pred.squeeze().squeeze()
    pred_nrm = cv2.normalize(pred, None, 255, 0, cv2.NORM_INF, cv2.CV_8UC1)
    # pred_nrm = cv2.equalizeHist(pred_nrm)
    pred_nrm = cv2.applyColorMap(pred_nrm, cv2.COLORMAP_INFERNO)
    cv2.imwrite('/data/btinaz/CARE_sample/nrm_sample_bs.png', pred_nrm)
    # plt.imsave('/data/btinaz/CARE_sample/sample_ft.png', pred.squeeze(), cmap='inferno')
    # plt.show()
